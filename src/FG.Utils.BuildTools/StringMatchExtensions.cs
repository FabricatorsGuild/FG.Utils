...
namespace FG.Utils.BuildTools
{
	public static class StringMatchExtensions
	{
		...
		public static string RemoveCommonPrefix(this string that, string compareTo, char componentSeparator, StringComparison stringComparison = StringComparison.InvariantCulture)
		{
			...
			while ((i < a.Length) && (j < b.Length))
			{
				...
				if (a_i.Equals(b_j, stringComparison))
				{
					result.Append(a_i);
		...
		public static string ToUpper(this Guid that)
		{
			return that.ToString().ToUpper();
		}
		public static string ToUpper(this Guid? that)
		{
			return that?.ToString().ToUpper();
